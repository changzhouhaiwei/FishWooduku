...
{	
	public class GUITools
	{
        ...
        static GUIStyle mStyle_ToolbarSearchTextField;
        static GUIStyle mStyle_ToolbarSearchCancelButton;
        static GUIStyle mStyle_ToolbarSearchCancelButtonEmpty;

        // Unity style names vary across versions/skins, so we resolve once and keep a safe fallback.
        public static GUIStyle Style_ToolbarSearchTextField
        {
            get
            {
                if (mStyle_ToolbarSearchTextField == null)
                    mStyle_ToolbarSearchTextField = ResolveToolbarStyle("ToolbarSeachTextField", "ToolbarSearchTextField", "SearchTextField", EditorStyles.textField);
                return mStyle_ToolbarSearchTextField;
            }
        }
        ...
        public static GUIStyle Style_ToolbarSearchCancelButton
        {
            get
            {
                if (mStyle_ToolbarSearchCancelButton == null)
                    mStyle_ToolbarSearchCancelButton = ResolveToolbarStyle("ToolbarSeachCancelButton", "ToolbarSearchCancelButton", "SearchCancelButton", GUIStyle.none);
                return mStyle_ToolbarSearchCancelButton;
            }
        }

        public static GUIStyle Style_ToolbarSearchCancelButtonEmpty
        {
            get
            {
                if (mStyle_ToolbarSearchCancelButtonEmpty == null)
                    mStyle_ToolbarSearchCancelButtonEmpty = ResolveToolbarStyle("ToolbarSeachCancelButtonEmpty", "ToolbarSearchCancelButtonEmpty", "SearchCancelButtonEmpty", GUIStyle.none);
                return mStyle_ToolbarSearchCancelButtonEmpty;
            }
        }

        static List<Action> mDelayedEditorCallbacks = new List<Action>();

        static GUIStyle ResolveToolbarStyle(string legacyName, string currentName, string altName, GUIStyle fallback)
        {
            var style = GUI.skin.FindStyle(legacyName) ?? GUI.skin.FindStyle(currentName) ?? GUI.skin.FindStyle(altName);
		...
		{
			// EditorGUILayout.BeginVertical(LocalizeInspector.GUIStyle_OldTextArea, GUILayout.MinHeight(10f));
			GUILayout.Space(2f);
