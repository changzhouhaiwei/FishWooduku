...
namespace I2.Loc
{
	...
	public class LocalizeInspector : Editor
	{
		...
		bool OnGUI_SelectKey( ref string Term, bool Inherited )  // Inherited==true means that the mTerm is empty and we are using the Label.text instead
		{
			...
			string popupLabel = (Index >= 0 && Index < mTermsArray.Length) ? mTermsArray[Index] : string.Empty;
			Rect popupRect = GUILayoutUtility.GetRect(new GUIContent(popupLabel), EditorStyles.popup, GUILayout.ExpandWidth(true));
			if (GUI.Button(popupRect, popupLabel, EditorStyles.popup))
			{
				TermSearchPopup.Show(popupRect, mTermsArray, Index, selected =>
				{
					mPendingTermIndex = selected;
					Repaint();
				});
			}

			int newIndex = Index;
			if (mPendingTermIndex >= 0)
			{
				newIndex = mPendingTermIndex;
				mPendingTermIndex = -1;
				GUI.changed = true;
			}
