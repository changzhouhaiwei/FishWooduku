    ...
    public class ActorEventDispatcher : IMemory
    {
        ...
        private void AddDelayDelete(int eventId)
        {
            if (!_delayDeleteEventList.Contains(eventId))
            {
                ...
                Debug.Log($"delay delete eventId[{eventId}]");
            }
        }
            ...
            if (_delayDeleteEventList.Contains(eventId))
            {
                if (_allEventListenerMap.TryGetValue(eventId, out var listListener))
                {
                    for (int i = 0; i < listListener.Count; i++)
                    {
                        ...
                        {
                            Debug.Log($"remove delay delete eventId[{eventId}]");
        ...
        public void SendEvent(int eventId)
        {
            if (_allEventListenerMap.TryGetValue(eventId, out var listListener))
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    {
                        Debug.LogError($"Invalid event data type: {eventId}");
                    }
                }


#if UNITY_EDITOR
                // Reporter.Log.Assert(iEventCnt == _processEventList.Count);
                // Reporter.Log.Assert(eventId == _processEventList[^1]);
#endif
        ...
        private void AddEventListenerImp(int eventId, Delegate listener, object owner)
        {
            ...
            if (existNode != null)
            {
                if (existNode.IsDeleted)
                {
                    ...
                    Debug.LogWarning($"AddEvent hashId deleted, repeat add: {eventId}");
                    return;
                }

                Debug.LogError($"AddEvent hashId repeated: {eventId}");
                return;
            }
