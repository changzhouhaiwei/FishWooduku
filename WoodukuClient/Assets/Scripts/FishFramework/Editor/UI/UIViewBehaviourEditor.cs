...
namespace FishFramework
{
    ...
    public abstract class UIViewBehaviourEditor : Editor
    {
        ...
        private ReorderableList CreateReorderableList(SerializedProperty opElementListSP)
        {
            ...
            reorderableList.drawHeaderCallback = (Rect rect) => { GUI.Label(rect, "OpElementList"); };
        ...
        private void RefreshOpElementList(ReorderableList list)
        {
            ...
            for (int i = 1; i < listSP.arraySize; i++)
            {
                for (int j = 0; j < i; j++)
                {
                    ...
                    if (targetSP_I.objectReferenceValue == null)
                    {
                        continue;
                    }

                    if (targetSP_J.objectReferenceValue == null)
                    {
                        continue;
                    }

                    if (!targetSP_I.objectReferenceValue.Equals(targetSP_J.objectReferenceValue))
                    {
                        continue;
                    }
        ...
        private void GenerateUIBaseCode()
        {
            ...
            content = content.Replace("${PanelLayer}", "public override PanelLayer Layer { set; get; }");
            content = content.Replace("${PrefabPath}", $"protected override string PrefabPath => \"{prefabPath}\";");
        ...
        private void GenerateUITempCode()
        {
            ...
            string subPath = Path.GetRelativePath(fullRootDir, fullPrefabPath);
            string className = Path.GetFileNameWithoutExtension(subPath);
            string subSavePath = Path.Combine(Path.GetDirectoryName(subPath) ?? string.Empty, className + ".cs");
        ...
        private int GetExportBaseCodeStrs(out string variantsDefineStr, out string bindCompsStr, out string bindEventsStr, out string unbindEventsStr, out string unbindCompsStr)
        {
            HashSet<string> canBindEventCompSet = new HashSet<string>()
            {
                ...
                "TMP_Dropdown", "TMP_InputField", "TButton", "TToggle"
            };

            string variantsDefineTempalte;
            string bindCompsLine;
            string bindEventsLine;
            string unbindEventsLine;
            string unbindCompsLine;
            ...
            for (int i = 0; i < uwb.opElementList.Count; i++)
            {
                ...
                if (opElement.componentList.Count == 1)
                {
                    variantsDefineTempalte = "protected ${CompType} ${GoName};";
                    bindCompsLine = "${GoName} = viewBehaviour.GetComponentByIndexs<${CompType}>(${i}, ${j});";
                    bindEventsLine = "BindEvent(${GoName});";
                    unbindEventsLine = "UnbindEvent(${GoName});";
                    unbindCompsLine = "${GoName} = null;";
                }
                else
                {
                    variantsDefineTempalte = "protected ${CompType} ${GoName}_${CompName};";
                    bindCompsLine = "${GoName}_${CompName} = viewBehaviour.GetComponentByIndexs<${CompType}>(${i}, ${j});";
                    bindEventsLine = "BindEvent(${GoName}_${CompName});";
                    unbindEventsLine = "UnbindEvent(${GoName}_${CompName});";
                    unbindCompsLine = "${GoName}_${CompName} = null;";
                }
